...
export const createdExportSlip = async (data) => {
  const { exportSlipCode, providerId, userId, status, products, newProducts, contracts, type, reason, exportPrice } = data;
...
export const searchExportSlip = async (exportSlipCode = "", providerId = "", agencyId = "", customerId = "", status = "", timeStart, timeEnd, page, limit, type) => {
  const res = await requestWithToken(authInstance, {
